...
(defvar devdocs-subjects nil
  "A place to cache the return value of the function `devdocs-subjects'.")

(defun devdocs-subjects (&optional refresh)
  "Return an alist of subjects supported by devdocs.io."
  (when (or refresh (not devdocs-subjects))
    (let ((url (format "%s/docs.json" devdocs-base-url)))
